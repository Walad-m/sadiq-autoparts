        ...
        <>
            ...
            <div className="flex flex-col gap-6 p-6">
                ...
                <div className="flex items-center gap-2 rounded-lg border border-input bg-background px-3 py-2">
                    ...
                    <input type="text" placeholder="Search categories..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="flex-1 bg-transparent text-sm outline-none placeholder:text-muted-foreground" />
                </div>
                    ...
                    <table className="w-full">
                        <thead className="border-b bg-muted">
                            <tr>
                                ...
                                <th className="p-4 text-left text-xs font-semibold uppercase tracking-wide">Description</th>
                                <th className="p-4 text-left text-xs font-semibold uppercase tracking-wide">Products</th>
                            ...
                            {filteredCategories.length > 0 ? (
                                filteredCategories.map((c) => (
                                    <tr key={c.id} className="border-t transition-colors hover:bg-muted/50">
                                        ...
                                        <td className="p-4 text-sm">{c.products_count ?? 0}</td>
                                        <td className="p-4">
                                            <div className="flex items-center gap-2">
                                                <Link href={`/categories/${c.id}/edit`} className="rounded p-1 text-muted-foreground transition-colors hover:bg-muted hover:text-foreground">
                                ...
                                <tr><td colSpan={4} className="p-8 text-center text-sm text-muted-foreground">No categories found.</td></tr>
                            )}
                        </tbody>
                    </table>
                </div>
